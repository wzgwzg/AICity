...
def _pluck_query_gallery(identities, indices, query_set, is_query, relabel=False):
    ret = []
    for index, pid in enumerate(indices):
        pid_images = identities[pid]
        for camid, cam_images in enumerate(pid_images):
...
class Dataset(object):
    ...
    @property
    def masks_dir(self):
        return osp.join(self.root, 'masks')
    ...
    def load(self, num_val=0.3, verbose=True):
        ...
        if 'market1501' not in self.root:
            self.query = _pluck(identities, self.split['query'])
            self.gallery = _pluck(identities, self.split['gallery'])
        else:
            query_list_path = osp.join(self.root, 'query.list')
            f = open(query_list_path)
            true_query_set = f.readlines()
            true_query_set = set([line.strip() for line in true_query_set])
            f.close()
            self.query = _pluck_query_gallery(identities, self.split['query'], true_query_set, True)
            self.gallery = _pluck_query_gallery(identities, self.split['gallery'], true_query_set, False)
