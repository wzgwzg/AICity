...
__all__ = ['multi_attribute_3_resnet50']
...
class Multi_Attribute_ResNet_3(nn.Module):
    ...
    def __init__(self, block, layers, num_classes, num_features, is_cls=True, test_attribute=False):
        ...
        self.cat_red_conv = nn.Conv2d(1024, self.num_features, 1, bias=False)
    ...
    def forward(self, x):
        ...
        cat_x = torch.cat([cls_x, type_x], 1)
...
def multi_attribute_3_resnet50(pretrained=False, **kwargs):
    print('type model')
    model = Multi_Attribute_ResNet_3(Bottleneck, [3, 4, 6, 3], **kwargs)
    return model
